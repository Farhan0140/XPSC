...
pair<ll, ll> Add_Check(pair<ll, ll> l, pair<ll, ll> r) {

    pair<ll, ll> tmp;

    tmp.first = min(l.first, r.first);
    tmp.second = 0;

    if(tmp.first == l.first) {
...
void Build (ll Node, ll Begin, ll End) {
    ...
    Tree[Node] = Add_Check(Tree[Left_Child], Tree[Right_Child]);
...
pair<ll, ll> Query(ll Node, ll Begin, ll End, ll i, ll j) {
    ...
    return Add_Check(Query(Left_Child, Begin, Mid, i, j), Query(Right_Child, Mid+1, End, i, j));
