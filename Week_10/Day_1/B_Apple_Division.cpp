...
void AppleDiv(vi arr, ll i, ll sum_1, ll sum_2, ll &ans, ll n) {
    if(i == n) {
        ans = min(ans, abs(sum_1 - sum_2));
        return;
    }

    AppleDiv(arr, i+1, sum_1+arr[i], sum_2, ans, n);
    AppleDiv(arr, i+1, sum_1, sum_2+arr[i], ans, n);
}
