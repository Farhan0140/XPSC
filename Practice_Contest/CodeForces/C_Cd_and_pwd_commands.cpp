...
void solve(int _c) {
    ...
    while(q--) {
        ...
        if(cmd == "pwd") {
            ...
            string folder; in(folder);
            string f_name = "";
            
            if(folder[0] == '.' && !dq.empty()) {
                dq.pop_back();

                for(int i=2; i<sz(folder); i++) {
                    if(folder[i] != '.' && folder[i] != '/') {
                        int j;
                        for(j=i; j<sz(folder); j++) {
                            if(folder[j] != '/' && folder[j] != '.') {
                                f_name += folder[j];
                            } else {
                                break;
                            }
