...
void solve(int _c) {
    ...
    ll node, vertex; in(node, vertex);
    vector<pair<ll, ll>> graph[node + 5];
    ...
    while( !pq.empty() ) {
        ...
        auto [p_cost, p_node] = pq.top(); pq.pop();
        if( p_cost > dis[p_node] ) {
            continue;
        }

        for(auto [c_cost, c_node]: graph[p_node]) {

            if( p_cost + c_cost < dis[c_node] ) {

                dis[c_node] = c_cost + p_cost;
