    ...
    private:
      ...
      void Propagate(ll Node, ll Begin, ll End) {
          ...
          if(Begin != End) {
              ...
              Lazy[ Left_Child ] = max( Lazy[ Left_Child ], Lazy[ Node ] );
              Lazy[ Right_Child ] = max( Lazy[ Right_Child ], Lazy[ Node ] );
      ...
      ll Query_Tree(ll Node, ll Begin, ll End, ll i, ll j) {
          ...
          return max( Query_Tree(Left_Child, Begin, Mid, i, j), Query_Tree(Right_Child, Mid + 1, End, i, j) );
