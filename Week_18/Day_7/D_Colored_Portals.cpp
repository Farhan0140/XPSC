...
void solve(int _c) {
    ...
    vector<string> arr(n);
    vector<vector<ll>> pos(6);

    for(int i=0; i<n; i++) {

        in(arr[i]);

        if(arr[i] == "BG") { pos[0].pb(i); }
        if(arr[i] == "BR") { pos[1].pb(i); }
        if(arr[i] == "BY") { pos[2].pb(i); }
        if(arr[i] == "GR") { pos[3].pb(i); }
        if(arr[i] == "GY") { pos[4].pb(i); }
        if(arr[i] == "RY") { pos[5].pb(i); }

    }

    while(q--) {

        ll i, j; in(i, j); i--; j--; if(j < i) { swap(i, j); }
        ...
        if(sz(st) < 4) {
            out(abs(i - j)); continue;
        }

        ll l = -1, r = n;
        for(int k=0; k<6; k++) {

            if(poss[k] == arr[i] || poss[k] == arr[j]) { continue; }

            auto left_pos = lower_bound(all(pos[k]), i);
            auto right_pos = upper_bound(all(pos[k]), i);

            if(left_pos != pos[k].begin()) {

                left_pos--;
                l = max(l, *left_pos);

            }
            if(right_pos != pos[k].end()) {
        ...
        if(l == -1 && r == n) { 
            ...
            out( min(abs(i-r) + abs(r-j), abs(i-l) + abs(l-j)) );
